...
var IntroMsg = "Hi, I'm Jaspal Chat! How may I help you?";
...
var PreChatSupportTermLbl = "<span class='agreed'><input type='checkbox' id='checkbox' value='' />:<p class='terms'>I agree to terms and conditions.</p></span>";
...
function InitReactive() {
    ...
    var loadChatUI = function (option) {
        ...
        var leftUI = "<img class='jp_logo' src='https://cdn.shopify.com/s/files/1/0069/1909/4327/files/home-treat.png?v=1698644408'>";
        leftUI += "<span class='heading_left'><img src='https://cdn.shopify.com/s/files/1/0069/1909/4327/files/header-text.png?v=1698656516' class='logo-text'/>Jaspal Group</span>";
        ...
        if (typeof option.middleURL == 'undefined') {
            curDlg = Dialog({ radius: [300, 400], nobg: true, bottom: "85px", right: "20px", header: true, dbody: option.content, dfooter: option.footer, maximize: false, left: left });
            left.style.display = "flex";
        }
        else {
            var content = "<iframe id='fChat' style='height:480px' src='" + option.middleURL + "' name='chat' ></iframe>";
            curDlg = Dialog({ radius: [400, 450], nobg: true, bottom: "85px", right: "20px", header: true, dbody: content, maximize: false, left: left });
        }
        curDlg.open();
        $(left).on("click", "li,.btn", function () {
            if ($("#fChat").length > 0 && $(this).attr("action"))
                document.getElementById("fChat").contentWindow.postMessage("{\"type\":\"" + $(this).attr("action") + "\"}", "*");
        });
    }
    if (sessionStorage.getItem("chat") == "active") {
        ...
        curDlg.body.innerHTML += "<div id='chat-frame'><form style='margin-top:30px;' id='" + TL_MsgFormId + "' name='sample' method='post' target='chat' action='about:blank'></form></div>";
        StartChat(false);
    }
    else {
        if (getQuery("instid") != "") {
            ...
            button.innerHTML = "<div class='bubble_chat' onclick='Opacity_change()'><img class='bubble_image' src='https://cdn.shopify.com/s/files/1/0069/1909/4327/files/home-treat.png?v=1698644408'></img><span class='inner_text'>Chat with us</span></div>";
            button.onclick = function () {
                if ($("#chat-frame").length == 0) {
                    ...
                    /*content +=
                        ...
                        content +=
                        "<div class='field'><input type='text' placeholder='JPS Club Member ID' id='21946' style='text-align:left' name='member_id'></input></div>";
                    ...
                    content +=
                        "<div class='field'><select id='21892' class='select_lang' style='text-align:left' name='reason' class='reason'> <option>Contact Reason</option> </select></div>";
                    ...
                    content +=
                        "<div class='field submit_button'><div class='field checkbox_field'><input required type='checkbox' placeholder='agree' value='' id='checkbox' style='text-align:left' name='terms_condition' class='terms_condition'></input><span class='agreement'>I agree to <a href='https://www.jpsclub.com/#/user_privacy' class='privacy_link' target='_blank'>JPSClub Privacy Policy, and Legal Statement</a></span></div><button onclick='StartChat(false);' id='btnSubmit' >" +
                        PreChatContinueButton +
                        "</button></div>";
                    content +=
                        "</form></div><iframe style='display:none;' id='fChat' src='' name='chat' ></iframe>";
                    var footer =
                        "<div class='left'><a class='btn'>" +
                        PreChatSupportTermLbl +
                        "</a></div>";
                    footer +=
                        "<div class='right'><a class='btn' onclick='StartChat(false,true);'>" +
                        PreChatSkipLbl +
                        "</a></div>";
                    curDlg.closeMe();
            ...
            curDlg = Dialog({ radius: [270, 30], nobg: true, bottom: "85px", right: "20px", noheader: true, dbody: ingro });
            setTimeout(function () {
                curDlg.open();
            }, 2000);
        }
...
function thai_options() {
    ...
    var newOptions = [
      ...
      {
            text: "คำสั่งซื้อ การเปลี่ยนแปลงและคืนสินค้า",
            value: "คำสั่งซื้อ การเปลี่ยนแปลงและคืนสินค้า",
        },
        { text: "สถานะการจัดส่งสินค้า", value: "สถานะการจัดส่งสินค้า" },
        { text: "โปรโมชั่น", value: "โปรโมชั่น" },
        { text: "สมาชิก JPSCLUB", value: "สมาชิก JPSCLUB" },
        { text: "ข้อมูลร้านค้าและอื่นๆ", value: "ข้อมูลร้านค้าและอื่นๆ" },
...
function eng_options() {
    ...
    var newOptions = [
        ...
        },
      {
        ...
        {
            text: "Orders, Change and Return Products",
            value: "Orders, Change and Return Products",
        },
        { text: "Shipping Status", value: "Shipping Status" },
        { text: "Promotions", value: "Promotions" },
        { text: "JPS Club", value: "JPS Club" },
        {
            text: "Store Information and Others",
            value: "Store Information and Others",
        },
...
function StartChat(proactive, ignorecheck) {
        ...
        var reason = document.forms["sample"]["reason"].value;
    ...
    if (OpenChat(proactive, ignorecheck)) {
        ...
        if (document.getElementById("chat-frame")) {
            ...
            curDlg.updateSize([460, 450]);
        }
        $(curDlg.footer).remove();
        $("#menu").show();
        curDlg.footer = null;
        $(curDlg.header).remove();
        curDlg.updateSize([400, 400]);
    }
        }
...
function LoadStyleFiles() {
    ...
    tjs.type = "text/css";
    tjs.rel = "stylesheet";
    tjs.href = "https://lynaccs.myshopify.com/cdn/shop/t/19/assets/treatcode_icon.css?v=164287151554408931451698724291";
    document.head.appendChild(tjs);
    var tjs1 = document.createElement("link");
    tjs1.type = "text/css";
    tjs1.rel = "stylesheet";
    tjs1.href = "https://lynaccs.myshopify.com/cdn/shop/t/19/assets/normalize.css?v=131714201630987549701699010020";
...
function LoadScript() {
    ...
    tjs.src = "//lynaccs.myshopify.com/cdn/shop/t/24/assets/jquery.js";
    document.head.appendChild(tjs);
    tjs.onload = function () {
        var tjs1 = document.createElement("script");
        tjs1.type = "text/javascript";
        tjs1.src = "//lynaccs.myshopify.com/cdn/shop/t/24/assets/jquery-ui.js";
        document.head.appendChild(tjs1);
        InitReactive();
...
function LoadStyle() {
    ...
    strStyle += ".chat-title {height:100px;width:100%;line-height:25px;cursor:pointer;}";
    ...
    strStyle += ".chat-body {width:100%;height: " + (ChatWindow_Height - 30) + "px;background-color:#fff;position:relative;}";
    strStyle += ".absolutes {position: fixed;right: " + chatFromRight + "px;bottom: 0px;z-index:100;}";
...
function LoadChatUI(isVideo) {
    ...
    bodyDiv.innerHTML = '<iframe style="display:none;" id="fChat" src="" name="chat" ></iframe>';
...
function EstimateTime() {
    ...
    if (xmlHttp.responseText != '') {
        ...
        var rsp = xmlHttp.responseText.replace(/{/g, "{\"").replace(/:/g, "\":\"").replace(/,/g, "\",\"").replace("}", "\"}");
        ...
        tjs1.onload = function () {
            ...
            tjs2.onload = function () {
                ...
                login(document.getElementById("VName"), function () {
                    ...
                    if (xmlHttp.responseText != "" && parseInt(xmlHttp.responseText) > 0) {
...
function Dialog(option) {
    ...
    var dg = false;
    var hasHeader = (typeof option.noheader == 'undefined' || option.noheader == false);
    var hasMaximize = (typeof option.maximize == 'undefined' || option.maximize == false) ? false : true;
    ...
    buttonClose.innerHTML = "<img src='https://cdn.shopify.com/s/files/1/0069/1909/4327/files/cancel-treat.png?v=1698644408'></img>";
    ...
    dialog.changeHeight = function (radius, firstTime) {
        ...
        if (typeof firstTime != 'undefined') {
            if (typeof option.bottom != 'undefined')
                dlg.style.bottom = option.bottom;
            if (typeof option.top != 'undefined')
                dlg.style.top = option.top;
            if (typeof option.left != 'undefined')
                dlg.style.left = option.left;
            if (typeof option.right != 'undefined')
                dlg.style.right = option.right;
        }
        setTimeout(function () {
    ...
    if (hasMaximize) {
        ...
        dialog.maximizeMe = function () {
            var isMax = (buttonMaximize.getAttribute("max") && buttonMaximize.getAttribute("max") == 'true') ? true : false;
            if (isMax == true) {
    ...
    if (typeof option.left != 'undefined')
        dialog.header.appendChild(option.left);
    dialog.header.appendChild(h2);
    dialog.header.appendChild(buttonClose);
    if (hasMaximize)
        dialog.header.appendChild(buttonMaximize);
    ...
    dialog.open = function () {
        ...
        if (dg == true)
            modalDg.className = "modal-bg show";
        if (typeof option.opened != 'undefined')
            option.opened();
    }
    dlg.focus();
    //$(dlg).draggable({
